...
# ── 2) 봇 서비스 구성 보장 (세 봇 모두 소액 실거래 / 신규 봇·타이머 설치) ──
# 사용자 결정: 대형70/잠수15/고위험15 비중으로 세 봇 모두 소액 실거래.
# (잠수함·고위험은 2019~2026 검증 미통과지만 소액으로 실험) → 잠수함봇에 --live 보장.
ensure_swing_live() {
    local S=/etc/systemd/system/swing-bot.service
    [ -f "$S" ] || return 0
    grep -q '^ExecStart=.*scripts.swing_trade' "$S" || return 0
    grep -q '^ExecStart=.*--live' "$S" && return 0   # 이미 실거래면 끝
    sed -i '/scripts\.swing_trade/ s/$/ --live/' "$S"
    systemctl daemon-reload
    systemctl restart swing-bot || true
    echo "  ✅ swing-bot 실거래(live) 전환 (--live 추가)"
}
...
ensure_clone_bot "highrisk-bot" "Upbit High-Risk Momentum Bot" \
    "scripts.highrisk_trade --live" "highrisk.log"   # 소액 실거래(--live)
