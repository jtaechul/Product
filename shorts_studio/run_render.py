...
씬 영상은 관리자 페이지가 Release 태그 `shorts-<id>` 에 `scene01.mp4` 형태로 올려 둔 것을 쓴다.
"""
...
def gh(path: str, token: str, method: str = "GET", data: bytes | None = None,
       content_type: str | None = None, base: str = API):
...
def download(url: str, token: str, dest: Path) -> Path:
    """Release 자산 내려받기. octet-stream을 요청해야 파일 본문이 온다."""
...
def main() -> int:
    ...
    tag = f"shorts-{cid}"
    rel = gh(f"/repos/{repo}/releases/tags/{tag}", token)
    assets = {a["name"]: a for a in rel.get("assets", [])}

    missing = [i for i in range(1, len(scenes) + 1) if f"scene{i:02d}.mp4" not in assets]
    if missing:
        print(f"::error::{', '.join(str(i) for i in missing)}번 씬 영상이 아직 올라오지 않았습니다.")
        return 1
    ...
    # 2. 씬 영상 내려받아 9:16 규격·길이 맞춤
    clips = []
    for i, dur in enumerate(durs):
        print(f"[{i + 1}/{len(scenes)}] 영상 규격 변환")
        raw = download(assets[f"scene{i + 1:02d}.mp4"]["url"], token,
    ...
    # 5. Release 업로드(같은 이름이 있으면 먼저 지운다)
    name = "final.mp4"
    if name in assets:
        gh(f"/repos/{repo}/releases/assets/{assets[name]['id']}", token, method="DELETE")
