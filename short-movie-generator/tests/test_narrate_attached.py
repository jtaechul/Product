...
def test_narrate_attached_e2e(tmp_path, monkeypatch, mode, w, h):
    ...
    assert "audio" in a
    # 메타데이터(제목·설명·해시태그, 일/한) 자동 생성 — 폴백이라도 채워짐
    meta = res["meta"]
    assert meta["title_jp"] and meta["desc_jp"] and meta["tags_jp"]
    assert Path(res["meta_path"]).exists()


def test_gen_metadata_fallback_and_llm(monkeypatch):
    """대본 → 제목·설명·해시태그(일/한). LLM JSON 우선, 실패 시 결정론 폴백."""
    ...
    assert fb["title_jp"] and fb["desc_jp"] and fb["tags_jp"] and fb["tags_ko"]
    good = ('{"title_jp":"深海の神秘","title_ko":"심해의 신비",'
