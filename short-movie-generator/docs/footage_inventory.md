...
> 실제 제작 경로로 굴려 **영상 확보 여부**로 분류. 영상은 '진짜 움직이는 클립'만(움직임≥8 · 느린 드리프트=moving image 제외).
> 찾은 영상 URL은 `_video_cache.json`에 캐시돼 제작 때 그대로 재사용(분류=제작 일치).


## 심해생물 (deep_sea) — 영상 3 · 이미지전용 18

### ✅ 영상+이미지 확보 (자동 제작 풀)

| # | 대상 | 영상 소스 |
|---|---|---|
| 1 | bathynomus | File:Bathynomus giganteus.webm |
| 2 | macrouridae | File:Ex1402-dive11 fish.webm |
| 3 | coelorinchus caelorhincus | NOAA OER EX1803 DIVE15 (https://www.ncei.noaa.gov/metadata/granu |

### 📷 이미지전용 · 영상 없음/부적합 (별도 보관)
...
| 16 | riftia pachyptila | - | 영상이 거의 정지(움직임 6.8<8)=moving image → 이미지전용 |
| 17 | chiridota heheva | - | 영상이 거의 정지(움직임 4.7<8)=moving image → 이미지전용 |
| 18 | thalassocalyce | - | 영상이 거의 정지(움직임 6.1<8)=moving image → 이미지전용 |

## 해양생물 (marine_life) — 영상 13 · 이미지전용 13
...
| 4 | carcharhinus melanopterus | File:Aerial view of a blacktip reef shark (Carcharhinus melanopt |
| 5 | rhinoptera bonasus | File:Cownose rays (Rhinoptera bonasus) Shark Reef Aquarium.webm |
| 6 | carcharhinus amblyrhynchos | File:Grey reef shark (Carcharhinus amblyrhynchos) near Sipadan.w |
...
| 9 | carcharias taurus | File:Requin taureau (Carcharias taurus), requin nourrice (Gingly |
...
| 12 | astomonema | - | 영상이 거의 정지(움직임 6.4<8)=moving image → 이미지전용 |
| 13 | phallodrilinae | - | 영상이 거의 정지(움직임 6.1<8)=moving image → 이미지전용 |

---
**합계: 영상 16 · 이미지전용 31 (총 47)** · 영상 URL 캐시 18종
