...
- **★첨부 영상 나레이션(운영자 확정 · 소싱 경로와 독립)**: 운영자가 **직접 받은 영상(퍼블릭도메인/CC 등
  ...
  - **★유튜브 썸네일 자동 생성(운영자 확정 · 쇼츠·롱폼 공통)**: 대본 근거로 **오프닝 훅 문구**(`hook_jp`)도
    ...
    - **★★표지(오프닝 훅·썸네일)는 로고·큰 글씨 프레임을 절대 쓰지 않는다(운영자 확정 · 회귀 금지)**:
      ...
      → `reframe.is_logo_frame` / `reframe.pick_clean_frames`(절대 임계 `LOGO_FRAME_TH=0.010`.
      실측: 로고 슬레이트 0.053 · 수중 프레임 0.0004 · 밝은 모래 0.000)로
      **①후보 프레임 필터(`_score_best_frame`) ②오프닝 폴백 ③엔드카드 폴백 ④히어로 사진**을 모두 검사한다.
      폴백은 `_grab_clean_frame`이 10~85% 구간을 여러 장 떠서 가장 깨끗한 것을 고른다.
      전부 더러운 소스면 **제작을 막지 않고** 가장 깨끗한 장으로 진행(경고 로그).
      ※ 화면 일부만 로고인 경우(구획)는 기존 `logo_box` delogo가 계속 담당한다.
      회귀: `tests/test_logo_frame_guard.py`(사고 재현 + 수정 확인).
