...
- **★첨부 영상 나레이션(운영자 확정 · 소싱 경로와 독립)**: 운영자가 **직접 받은 영상(퍼블릭도메인/CC 등
  ...
  - **★유튜브 썸네일 자동 생성(운영자 확정 · 쇼츠·롱폼 공통)**: 대본 근거로 **오프닝 훅 문구**(`hook_jp`)도
    ...
    - **★자막 스크립트 확인(운영자 요청)**: 화면에 나가는 자막을 `meta["subs"]`(=[{s,e,jp,ko}])로 남겨
      레코드에 저장하고, 관리자 상세에 **시각 · 일본어 · 한국어 표**로 보여준다(`subsPanel`).
      시각은 **완성본 기준**(오프닝 훅 길이만큼 밀어서 기록). 한국어는 1회 LLM 번역, 실패 시 빈칸(발행 불정지).
      구독 유도 줄은 표에서 강조 표시한다 — 눈으로 '마지막에 들어갔는지' 바로 확인하기 위함.
