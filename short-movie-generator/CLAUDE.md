...
- **★첨부 영상 나레이션(운영자 확정 · 소싱 경로와 독립)**: 운영자가 **직접 받은 영상(퍼블릭도메인/CC 등
  ...
  - **★유튜브 썸네일 자동 생성(운영자 확정 · 쇼츠·롱폼 공통)**: 대본 근거로 **오프닝 훅 문구**(`hook_jp`)도
    ...
    - **★오프닝 훅 '영상 카드'는 폐지(운영자 확정 · 실사고 없음 · 순수 개선)**: 예전엔 같은 훅 문구를
      정지 타이틀 카드(~2.8초, 무음·페이드인)로 본문 앞에도 붙였다. 썸네일이 이미 클릭 유인을 담당하므로
      **영상 시작에 같은 문구를 또 보여주는 건 중복**이고, 재생 시작 직후 '아무 일도 안 일어나는' 구간은
      초반 이탈(retention)에 불리하다 → **본문이 곧바로(0초부터) 시작**한다. 썸네일 렌더는 그대로
      유지(카드 이미지 자체는 만들되 영상에는 붙이지 않음). 옛 `_build_hook_clip`/`_concat_av`(무음 인트로
      클립 생성·concat)는 사용처가 없어져 삭제. 롱폼 설명란 챕터 타임스탬프도 더는 훅 길이만큼 오프셋을
      더하지 않는다(항상 0초 기준).
