...
def detect_text_bands(frame_paths: list[str], bands: int = 40) -> tuple[float, float]:
    ...
    # 피사체**(예: 킹크랩)의 텍스처 에지를 텍스트로 오검(빈 물·얇은 띠로 과크롭)했다. 실측:
    # 밝은 모래+킹크랩 컷 창들의 피크 밀도 0.035~0.042 vs 실제 번인 텍스트(로고·URL·타이틀카드)
    # 0.12~0.14. 0.040은 모래 상한(0.042) 바로 아래라 여전히 오검됐다 → **0.070**으로 올려
    # 모래(≤0.042)와 텍스트(≥0.12)를 확실히 분리(양쪽 1.7배 여유). 밝은배경 클립 얇은띠 재발방지.
    HIT = 0.070
...
# ★4 → 3(운영자 확정 · 시청자층 대응): 시청자의 71%가 45세 이상(65+ 만 27%)이다.
#   이 연령대는 빠른 구도 전환에서 이탈이 커, 컷을 하나 더 줄이고 한 컷을 길게 본다.
_DEFAULT_MAX_CUTS = 3     # 기본 컷 수(=전환 2회)
