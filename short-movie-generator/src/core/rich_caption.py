...
# ★모든 해시태그에 공통 태그를 항상 포함(일본어). 한국어판은 대응 번역.
_CORE_TAGS_JP = ("#深海", "#海洋生物")
_CORE_TAGS_KO = ("#심해", "#해양생물")


def _with_core_tags(tags: list[str], core: tuple) -> list[str]:
    """공통 태그(深海·海洋生物 등)를 항상 앞에 포함하고, 기존 태그를 뒤에 이어붙여 중복 제거."""
    out = list(core)
    for t in (tags or []):
        s = str(t).strip()
...
def _fallback(info: SpeciesInfo, jp_name: str, sci_name: str, feature_line: str,
    ...
    default_tags = _with_core_tags(default_tags, _CORE_TAGS_JP)
    tags_ko = _with_core_tags(tags_ko, _CORE_TAGS_KO)
...
def generate(info: SpeciesInfo, jp_name: str, sci_name: str, feature_line: str,
    ...
    if _is_rich(jp):
        ...
        return {"jp": jp, "ko": ko,
                "tags": _with_core_tags(tags, _CORE_TAGS_JP),
                "tags_ko": _with_core_tags(ko_tags, _CORE_TAGS_KO),
                "yt_title": tj, "yt_title_ko": tk}
