...
- 日本語 **18〜32字**。**ハッシュタグ(#Shorts等)・絵文字・記号装飾は一切付けない**
  (ショートは形式で自動判定される。タグは文字数の無駄でスパム印象を与える)。
- **冒頭10〜12字に一番強いフック**を置く(フィードでは先頭しか見えない)。
- 核は**好奇心ギャップ**: 一番意外な事実を「答えを言わずに」チラ見せ。型を毎回変える:
  ①逆説「AなのにB」(例: 骨がないのに、泳ぐ) ②疑問「なぜ/どうやって〜のか」
  ③具体数字・事実のチラ見せ(例: 水深1000mで“歩く”) ④謎かけ体言止め(〜の正体)。
- **【衝撃】【驚愕】「99%が知らない」等の汎用煽り接頭辞は禁止** — 抽象的な煽りでなく
  「確かな事実」の中の具体的な意外性で釣る(信頼が資産のチャンネル)。
- 検索性のため**種名({jp})を含める**(フックの後ろでよい)。誇張・捏造は禁止。
...
def _valid_title(t: str, jp_name: str) -> bool:
    """LLM 제목 채택 기준: 정리 후 10~45자 + 종명 포함(검색성)."""
    t = _clean_title(t)
    return bool(t) and 10 <= len(t) <= 45 and (jp_name in t if jp_name else True)
