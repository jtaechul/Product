...
- **深海の極限環境(暗黒・超水圧など)と、その生物の奇妙な生存戦略を劇的に対比**させる構造。
- 「衝撃の進化」「透明マント」「深海の怪物」「ゾッとする」等、想像を掻き立てる**強いフックワードを実在特徴に絡めて**組み合わせる。
- **全角30字以内**で簡潔に。**モバイルで切れないよう、最も刺激的な核キーワードを必ず文頭に置く**(先頭しか見えない)。
- **ハッシュタグ(#Shorts等)・絵文字・記号装飾は一切付けない**(ショートは形式で自動判定。タグは無駄でスパム印象)。
- 検索性のため**種名({jp})を含める**(フックの後ろでよい)。
- **【衝撃】【驚愕】「99%が知らない」等の“中身のない”汎用接頭辞は禁止**(具体性のある刺激語で釣る)。
