...
# ★정지-이미지 소스 차단(기본 문턱): 원본이 '움직이지 않는 정지 화면'이면 영상 제작 금지.
# 실측: 실제 수중 영상 median 밝기차 13~36, 정지 0~1 → 3.0 아래면 정지. ★이 기본값은 사진 켄번즈
# (photo-doc)도 통과해야 하므로 낮게 유지한다(켄번즈 모션 ~6). '진짜 움직이는 영상'만 원할 땐 호출부가
# threshold를 높여 부른다 — 실사 영상 소스 게이트는 `footage._MIN_VIDEO_MOTION`(8.0)으로 강화(느린
# 심해 드리프트=moving image 배제). 재발방지: "그냥 moving image지 진짜 영상 아니다".
