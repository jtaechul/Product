...
**★常体(だ・である/〜する/〜だ)は禁止。敬体(です・ます)または名詞・体言止め・疑問形で終える**\
(例: 「〜でしょうか」「〜タコ?」)。ぞんざいな言い切りにしない
...
要件:
- **まず流れを設計してから書く**: 導入(情景)→驚き→確かな事実→感情的な余韻、という起承転結を頭で組んでから、
  ぶつ切りの箇条書きではなく**自然でなめらかな文**に落とす(棒読みにならないように)。
- **★敬体(です・ます)で書く。常体(だ・である/〜する/〜いる)は禁止**(視聴者に語りかける丁寧な口調)。
  例: 「〜しています」「〜のです」「〜でしょうか」。名詞・体言止めの短い一節は可(丁寧さを保つ範囲で)。
- 各要素は読点や句点で終わる**短めの節**にし、字幕同期しやすくする。
- **実際の形態・行動のみ**。無い発光・捕食・危険・数値の捏造禁止。
- 序盤で正体(和名)を明かし、中盤で驚きの特徴、終盤で感情的な余韻。
出力例: ["深海の底に、","じっと動かない母がいます。","その理由は、卵を守るため。", ...]
"""
