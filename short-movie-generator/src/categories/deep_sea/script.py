...
_PROMPT = (
    ...
    " 그것을 '역설/결핍 훅 → 2단 리빌 → 재프레이밍'으로 공개한다.\n"
    "[구조 — 정확히 6문장, 현재형 단문, 각 30자 이내, 갈수록 고조]\n"
    "1) 훅(역설/결핍형): 이 종이 '남들과 달리 다르게 하는 것' 또는 '실제로 없는 것'을 던져 호기심을 연다.\n"
    "   ★정확성: 실제로 참인 결핍만. '헤엄 못 침/도망 못 침/무방비/생존 불가' 같은 거짓 단정 금지.\n"
    "   (먹물주머니가 진짜 없으면 '먹물이 없다'는 OK. 지느러미로 이동하면 '남들처럼 제트로 헤엄치지 않는다'로.)\n"
    "2) 긴장 도입: 그 사실이 놓인 극한 환경을 짧게 연다(어둠·수압·거리).\n"
    "3) 1차 리빌(공개): 시그니처 실제 행동 + 극한 실측 수치 1개(수심·속도·시간, 아래 데이터에 있는 값만).\n"
    "4) 2차 리빌: 숨은 두 번째 실제 사실(감각·생존기제, 또는 '오래 미지였다 관측된 순간')을 쌓는다.\n"
    "5) 재프레이밍: 낯설거나 섬뜩한 사실을 '경이'로 뒤집는다.\n"
    "6) 시적 마무리: 사실을 감정으로 승화하는 여운 한 문장(예: '여기선 아무것도 보이는 그대로가 아니다').\n"
    "[규칙]\n"
    "- 아래 '검증된 사실'만 사용. 없는 행동·수치·관계·위험·포식·발광 날조 절대 금지.\n"
    "- 결핍/역설 훅도 반드시 사실. 극적 효과를 위해 거짓 단정(못 한다/없다)을 지어내지 말 것.\n"
    "- 수치는 아래 데이터에 실제로 있는 것만. 없으면 수치 문장을 넣지 말 것.\n"
