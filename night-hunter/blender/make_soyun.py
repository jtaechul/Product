"""
make_soyun.py — 소윤 chibi 3D 캐릭터 생성 후 GLB로 내보내기
실행: blender --background --python make_soyun.py
출력: output/soyun.glb
"""
import bpy
import os

OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "output", "soyun.glb")

# 팔레트 (소윤)
SKIN   = (1.00, 0.88, 0.78, 1)
HAIR   = (0.35, 0.23, 0.10, 1)
EYE    = (0.48, 0.30, 0.16, 1)
UNIF   = (0.12, 0.19, 0.38, 1)
GOLD   = (0.83, 0.66, 0.13, 1)
BLACK  = (0.07, 0.07, 0.07, 1)
LIP    = (0.91, 0.64, 0.56, 1)
...
def add_mesh(name, verts, faces, color, loc=(0,0,0)):
...
for sign, prefix in [(-1,"L"), (1,"R")]:
    ...
    sphere(f"eye_p_{prefix}", 0.05, (sign*0.19,  0.505,1.91), BLACK)
...
print(f"[make_soyun] 내보내기 완료: {OUTPUT_PATH}")
