    ...
    const ChibiCharacter = {
        ...
        preload() {
            ...
            this._promise = Promise.all([
                ...
                safe(loadGLB('assets/models/soyun.glb?v=4').then(g => { meshCache['soyun'] = g; }), 'soyun'),
                // 남자 NPC 모델 (동일 스켈레톤 → idle/walk/run 클립 공유).
                // 텍스처를 1024²로 줄여(약 4MB) iPad 메모리 초과를 방지함. 실패해도 NPC는 기존 캐릭터로 폴백.
                safe(loadGLB('assets/models/npc-youngman.glb?v=4').then(g => { meshCache['npc-man'] = g; }), 'npc-man'),
                // 납치범(suspect) NPC 모델 (동일 스켈레톤, 텍스처 1024²).
                safe(loadGLB('assets/models/npc-kidnapper.glb?v=4').then(g => { meshCache['kidnapper'] = g; }), 'kidnapper'),
                // 일반 수배범(밤 수배범) 모델 (동일 스켈레톤, 텍스처 1024²).
                safe(loadGLB('assets/models/npc-suspect.glb?v=4').then(g => { meshCache['suspect'] = g; }), 'suspect'),
                // 여자 시민 NPC 모델 (동일 스켈레톤, 텍스처 1024²).
                safe(loadGLB('assets/models/npc-youngwoman.glb?v=4').then(g => { meshCache['woman'] = g; }), 'woman'),
                // 공통 애니메이션 (root motion 제거 → 제자리 재생). 실패해도 정적 캐릭터로 표시.
                safe(loadGLB('assets/models/idle.glb?v=4').then(g => {
                    if (g.animations.length) animCache['idle'] = stripHorizontalRootMotion(g.animations[0]);
                    disposeGLTFScene(g);   // 메시·텍스처 해제 (애니 클립만 사용)
                }), 'idle'),
                safe(loadGLB('assets/models/walk.glb?v=4').then(g => {
                    if (g.animations.length) animCache['walk'] = stripHorizontalRootMotion(g.animations[0]);
                    disposeGLTFScene(g);
                }), 'walk'),
                safe(loadGLB('assets/models/run.glb?v=4').then(g => {
