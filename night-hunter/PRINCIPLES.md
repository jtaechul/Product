...
6. **횡단보도는 교차로 4면(북/남/동/서) 의 인도 끝에만 배치.**
   ...
   - zebra 패턴: 흰 띠 6개, 폭(l1) 0.5m, 간격(l2) 0.7m, **줄무늬는 도로(차량 진행) 방향과
     평행하게 길게 배치하고, 줄들은 보행 방향(도로를 가로지르는 방향)으로 나열한다**
     (Abbey Road 식 — 보행자가 줄을 하나씩 밟고 건너는 방향과 줄의 긴 변이 직교).
   - **횡단보도 밴드는 반드시 실제 도로 asphalt 구간(offsetZ/length 또는
     offsetX/length 범위) 내부에만 그린다** — 도로가 끝나는 지점(막다른 끝)
     근처 교차로에서는 도로 밖으로 밴드가 넘치지 않는지 검사 후, 넘치면
     해당 면의 횡단보도를 생략한다 (인도/잔디 위에 횡단보도가 그려지면 안 됨).

## 부분 구간 도로 (offsetX / offsetZ)

6-1. **H 도로에 `offsetX`, V 도로에 `offsetZ` 를 사용해 전체 폭이 아닌 부분
     구간 도로를 정의할 수 있다.** 이 경우 다음을 모두 갱신해야 한다:
   - `validateBlocksVsRoads` 의 AABB 계산 (roadMinX/MaxX 또는 roadMinZ/MaxZ)
   - `createRoadNetwork` 의 메쉬 위치/길이 계산과 횡단보도 교차 판정(`inH`/`inV`)
   - `createStreetProps` 의 `lightOnRoad` / `onAnyRoad` / 가로수·가로등 배치 루프
   - `ui.js` 의 `updateMinimap`, `drawFullMap` 도로 라인 렌더링
   - 누락 시 부분 구간 밖에 소품·횡단보도가 그려지거나 검증이 실패할 수 있음.
