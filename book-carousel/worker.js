...
async function handleSuggest(env, body) {
  ...
  const suggestSystem = isStory
    ? '당신은 로맨스·연애 소설 전문 큐레이터입니다. 두 사람의 사랑·썸·짝사랑·이별·재회가 이야기의 "중심 줄기"인 실제 출판된 연애(로맨스) 소설만 추천합니다. 인물의 관계 패턴이 뚜렷해 심리적으로 읽어낼 여지가 있는 작품을 고릅니다. [절대 금지] 사회비판·가족서사·성장·역사·미스터리 등 연애가 곁가지인 순수문학/일반문학은 제외(예: 82년생 김지영·아몬드처럼 연애가 중심이 아닌 작품 금지). 성인용·19금·에로/관능 소설도 절대 금지. 반드시 JSON만 응답합니다.'
    : '당신은 "연애 책방" 큐레이터입니다. 30대 독자가 자신의 연애·관계를 더 깊이 이해하도록, 폭넓은 실제 출판 도서(베스트셀러 포함 — 심리·에세이·인문·자기계발·소설 등)에서 그 책의 "진짜 핵심 메시지"를 연애·관계의 관점으로 정직하게 연결할 수 있는 책을 추천합니다. 핵심 원칙: 억지·할루시네이션 금지 — 책이 실제로 말하는 통찰을 연애 상황에 "적용"하는 것이지, 그 책이 연애책인 척 지어내는 게 아닙니다(예: 습관/소통/경계/자존감/상실/용기에 관한 책 → 연애에서의 같은 통찰). [금지] 성인용 만화·성인 소설·에로/19금·관능 콘텐츠는 절대 추천 금지. 반드시 JSON만 응답합니다.';
  ...
  if (usable.length > 1 && isStory) {
    try {
      ...
      const rgText = await callLightModel(env, {
        ...
        user: `아래 소설들이 "두 사람의 연애(사랑·썸·짝사랑·연인·이별·재회)가 이야기의 중심 줄기인 로맨스/연애 소설"인지 각 소개문 기준으로 판정하세요. 연애가 곁가지이고 사회비판·가족·성장·역사·미스터리·자기성찰이 중심이면 false(예: 82년생 김지영·아몬드·채식주의자 유형은 false).\n\n${items}\n\nJSON: {"fits":[true,false,...]} (순서대로 정확히 ${usable.length}개)`,
      });
