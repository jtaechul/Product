...
async function handleSuggest(env, body) {
  ...
  const suggestSystem = isStory
    ...
    : '당신은 연애·관계 심리 전문 도서 큐레이터입니다. 30대 독자가 자신의 연애 패턴·이별·짝사랑·애착 유형을 이해하도록 돕는 실제 출판된 책을 추천합니다. 사랑·관계·심리·자기이해를 다루는 에세이, 심리학, 관계 안내서를 중심으로 큐레이션합니다. [금지] 성인용 만화·성인 소설·에로/19금·관능 소설 등 성인용(19세 이상) 콘텐츠는 절대 추천하지 마세요. 건전한 일반 도서만 추천합니다. 반드시 JSON만 응답합니다.';
  ...
  // 단, 소설 레인은 소개가 "줄거리"라 이 게이트(주제 직접 다룸)가 소설을 통째로 탈락시키므로 생략.
