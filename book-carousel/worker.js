...
// ⭐ 동성애(BL·GL·퀴어) 로맨스 필터 — 계정 콘셉트(30대 이성 연애 심리)와 맞지 않아
// 추천·제작 어느 단계에서도 절대 나오지 않도록 하드 차단한다. (오탐보다 누락 방지 우선)
const SAMESEX_ROMANCE_PATTERNS = [
  /동성\s*애/, /동성\s*(연애|간|커플|로맨스|사랑)/, /동성애자/,
  /\bBL\b/i, /\bGL\b/i, /비엘\s*(소설|로맨스|물)?/, /보이즈\s*러브/, /걸즈\s*러브/, /야오이/,
  /백합물/, /백합\s*(로맨스|소설|장르)/, /퀴어/i, /\bqueer\b/i,
  /레즈비언/i, /\blesbian\b/i, /게이\s*(소설|로맨스|romance|물)/i,
  /\bgay\b.{0,14}(romance|novel|love|fiction|사랑|연애|소설)/i,
  /남남\s*(커플|로맨스|연애|사랑)/, /여여\s*(커플|로맨스|연애|사랑)/,
  /두\s*남자(의|\s)*(사랑|로맨스|연애)/, /두\s*여자(의|\s)*(사랑|로맨스|연애)/,
];
function isSameSexRomance(...parts) {
  const t = parts.filter(Boolean).join(' ');
...
async function handleSuggest(env, body) {
  ...
  // 프론트가 보낸 제외 목록 + "도서관에 실제 게시된 책"만 제외(제작만 한 책은 다시 추천 가능).
  ...
  const suggestSystem = isStory
    ? '당신은 로맨스·연애 소설 전문 큐레이터입니다. "남녀(이성) 간의 사랑·썸·짝사랑·이별·재회"가 이야기의 "중심 줄기"인 실제 출판된 연애(로맨스) 소설만 추천합니다. 인물의 관계 패턴이 뚜렷해 심리적으로 읽어낼 여지가 있는 작품을 고릅니다. [절대 금지] ① 동성애(BL·GL·퀴어·백합·보이즈러브)를 소재로 하는 소설은 어떤 경우에도 추천 금지. ② 사회비판·가족서사·성장·역사·미스터리 등 연애가 곁가지인 순수문학/일반문학 제외(예: 82년생 김지영·아몬드처럼 연애가 중심이 아닌 작품 금지). ③ 성인용·19금·에로/관능 소설 금지. 반드시 JSON만 응답합니다.'
    : '당신은 "연애 책방" 큐레이터입니다. 30대 독자가 자신의 연애·관계를 더 깊이 이해하도록, 폭넓은 실제 출판 도서(베스트셀러 포함 — 심리·에세이·인문·자기계발·소설 등)에서 그 책의 "진짜 핵심 메시지"를 연애·관계의 관점으로 정직하게 연결할 수 있는 책을 추천합니다. 핵심 원칙: 억지·할루시네이션 금지 — 책이 실제로 말하는 통찰을 연애 상황에 "적용"하는 것이지, 그 책이 연애책인 척 지어내는 게 아닙니다(예: 습관/소통/경계/자존감/상실/용기에 관한 책 → 연애에서의 같은 통찰). [절대 금지] 동성애(BL·GL·퀴어) 소설·콘텐츠, 성인용 만화·성인 소설·에로/19금·관능 콘텐츠는 절대 추천 금지. 반드시 JSON만 응답합니다.';

  const suggestUser = isStory
    ? `오늘 날짜: ${today}\n주제: 순수 로맨스·연애 소설(두 사람의 사랑이 중심인 이야기)${excludeClause}\n\n30대 독자가 인물의 연애 감정에서 "이건 내 얘기 같다"를 느낄 실제 로맨스/연애 소설 ${count}권을 추천하세요.\n\n[필수 — 실존 도서만] 실제로 한국에 출간된 것을 확신하는 소설만 추천하세요. 제목·저자를 지어내지 마세요(가짜는 시스템에서 자동 탈락). 국내 로맨스·번역 로맨스·웹소설 단행본 등 폭넓게 떠올려 확실한 것만 채우세요.\n\n[선정 기준 — 순수 연애소설만]\n- 두 사람의 연애(사랑·썸·짝사랑·연인·이별·재회)가 이야기의 "중심 축"인 로맨스 소설일 것. 연애가 곁가지면 제외.\n- [반드시 제외] 사회비판·가족·성장·역사·직업·미스터리·자기성찰이 주제인 순수/일반문학(예: 82년생 김지영, 아몬드, 채식주의자 같은 유형)은 유명해도 절대 넣지 말 것.\n- 인물의 관계 패턴(회피·불안·집착·헌신·밀당 등)이 뚜렷해 심리로 읽어낼 여지가 있을 것.\n- 너무 무겁지 않고 몰입감 있는, 30대가 부담 없이 읽을 작품. 국내 출간·번역 로맨스 우선. 동일 저자 중복 금지.\n- 성인/19금/과도하게 자극적인 작품 금지.\n\n각 책에 대해:\n- title: 소설 제목 (실제 출판된 정확한 제목)\n- author: 저자명 (정확히)\n- year: 출판연도 (숫자)\n- category: 세부 갈래 (예: 로맨스 / 연애소설 / 로맨스판타지)\n- coreMessage: 이 소설이 그리는 사랑·연애의 마음 (1~2문장, 결말 스포일러 금지)\n- targetAudience: 주요 대상 독자층 (1문장)\n- reason: 30대가 지금 이 소설을 읽으면 좋은 이유 (1문장)\n- lesson: 이 이야기 속 인물의 연애 패턴을 애착·심리로 읽어낸 한 문장 통찰 (마음에 남는)\n\nJSON 형식:\n{"books":[{"title":"...","author":"...","year":2024,"category":"...","coreMessage":"...","targetAudience":"...","reason":"...","lesson":"..."}]}`
    : `오늘 날짜: ${today}\n연애 주제(톤): "${topic}"${excludeClause}\n\n30대 독자가 자신의 연애·관계를 돌아보게 할 실제 책 ${count}권을 폭넓게 추천하세요. 꼭 "연애책"일 필요는 없습니다 — 베스트셀러 포함 다양한 분야(심리·에세이·인문·자기계발·소설 등)에서, 그 책의 진짜 통찰을 "${topic}"의 연애 관점으로 정직하게 연결할 수 있으면 됩니다.\n\n[필수 — 실존 도서만] 실제로 한국에 출간된 것을 확신하는 책만. 제목·저자를 지어내지 마세요(가짜는 시스템에서 자동 탈락). 확신 없으면 빼고 확실한 책으로 채우세요.\n\n[선정 기준 — 풀을 넓게]\n- 분야를 가리지 말 것(연애 심리서에 국한하지 않음). 베스트셀러·스테디셀러·화제작 환영.\n- 단, 그 책의 "실제 핵심 메시지"를 연애·관계에 억지 없이 연결할 수 있어야 함(할루시네이션 금지 — 책에 없는 내용을 지어내 붙이지 말 것). 연결이 억지스러우면 그 책은 넣지 말 것.\n- 예: 소통·경청에 관한 책 → 연인과의 대화 / 습관에 관한 책 → 관계의 반복 패턴 / 경계·거절에 관한 책 → 건강한 연애 거리 / 상실·애도에 관한 책 → 이별 회복 / 자존감에 관한 책 → 나를 지키는 사랑.\n- 한국 독자가 쉽게 구할 수 있는 국내 출간서 우선. 동일 저자 중복 금지.\n\n각 책에 대해:\n- title: 책 제목 (실제 출판된 책, 정확한 제목)\n- author: 저자명 (정확히)\n- year: 출판연도 (숫자)\n- category: 세부 분야 (예: 심리 / 에세이 / 인문 / 자기계발 / 소통 / 소설 등 실제 분야)\n- coreMessage: 이 책의 진짜 핵심 메시지 (1~2문장, 책 실제 내용 기준)\n- targetAudience: 주요 대상 독자층 (1문장)\n- reason: 30대가 지금 이 책을 읽으면 좋은 이유 (1문장)\n- loveAngle: 이 책의 통찰을 "${topic}"의 연애 관점으로 연결하는 정직한 한 문장 다리(책 내용에서 실제로 도출되는 연결만)\n- lesson: 위 연결에서 나오는, 마음에 남는 연애 통찰 한 문장\n\nJSON 형식:\n{"books":[{"title":"...","author":"...","year":2024,"category":"...","coreMessage":"...","targetAudience":"...","reason":"...","loveAngle":"...","lesson":"..."}]}`;

  const text = await callClaude(env.ANTHROPIC_API_KEY, {
    env, tier: 'main', max_tokens: Math.min(3400, 600 + count * 340), // 후보 수에 비례(잘림 방지 하한 포함)
  ...
  // 연애·관계 관점으로 "억지·할루시네이션 없이" 연결할 수 있는지만 본다. 예전처럼 "주제를 직접
  // 다루는 책"만 남기지 않는다(그건 풀을 말려버림). 연결이 명백히 억지스러운 책만 탈락.
  // 소설 레인은 별도 로맨스 게이트가 처리하므로 여기선 생략.
  if (usable.length > 1 && !isStory) {
    try {
      ...
      const ftText = await callLightModel(env, {
        ...
        user: `연애 관점: "${topic}"\n\n아래 각 책의 "실제 소개문"을 볼 때, 그 책의 진짜 통찰을 연애·관계(${topic})의 관점으로 "억지나 왜곡 없이 정직하게" 연결할 수 있으면 true, 연결이 명백히 억지스럽거나 책 내용을 지어내야만 가능하면 false로 판정하세요. (책이 꼭 연애책이 아니어도, 통찰이 연애에 자연스럽게 적용되면 true)\n\n${items}\n\nJSON: {"fits":[true,false,...]} (순서대로 정확히 ${usable.length}개)`,
      });
  ...
  // ⭐ 로맨스 게이트(소설 레인 전용) — 네이버 소개(줄거리)를 기준으로 "연애가 중심인
  // 로맨스 소설"만 남긴다. AI가 유명세로 순수/일반문학(82년생 김지영·아몬드 등 연애가
  // 곁가지인 작품)을 끼워 넣어도 여기서 걸러낸다.
  if (usable.length > 1 && isStory) {
    try {
      ...
      if (Array.isArray(rf.fits) && rf.fits.length === usable.length) {
        const romance = usable.filter((_, i) => rf.fits[i] === true);
...
async function handleGenerate(env, body) {
  ...
  // ⭐ 동성애(BL·GL) 필터(1차) — skipVerify로도 우회 불가. 이 계정 콘셉트와 맞지 않아 제작 금지.
  if (isSameSexRomance(title, author, category, coreMessage)) {
    throw new Error('SAMESEX_BLOCKED: 동성애(BL·GL·퀴어) 소설은 이 계정에서 제작하지 않습니다.');
  }
  ...
  if (!body.skipVerify) {
    ...
    // ⭐ 동성애 필터(2차) — 네이버 실제 소개글로 재확인(제목만으론 안 걸리는 BL·GL 차단).
    if (isSameSexRomance(cv.title, cv.description)) {
      throw new Error('SAMESEX_BLOCKED: 동성애(BL·GL·퀴어) 소설은 이 계정에서 제작하지 않습니다.');
    }
    if (cv.status === 'found') {
      bookCover = cv.cover || '';
...
function laneOf(cat) {
  const c = String(cat || '');
  // story = 소설/이야기 속 인물의 관계를 심리로 읽는 레인(A안). 다른 레인보다 먼저 판정.
  if (/소설|이야기|로맨스|인물|캐릭터|드라마\b/.test(c)) return 'story';
...
const LANE_TONES = {
  ...
  // ⭐ story(A안) — 연애 소설·이야기 속 인물을 미끼로, 알맹이는 심리 통찰.
  // 소설의 몰입감(가벼움·공유력)을 빌리되, 여전히 "내 마음을 읽어주는" 저장가치를 남긴다.
  story: {
    audience: '가볍게 연애 이야기·소설을 즐기며 그 속에서 자기 마음을 발견하고 싶은 30대',
    theme: '이야기 속 연애와 마음',
    tone: '이야기의 몰입감 + 부드러운 심리 통찰 — 무겁지 않게, 소설 속 그 사람의 마음을 빌려 "이거 내 얘기 같다"를 느끼게 한다. 결말·반전 스포일러 금지, 인물을 비난하지 않는 따뜻한 시선',
    flow: '이야기 속 인물의 관계 장면·감정에 공감 → 그 인물이 그렇게 사랑하거나 떠난 심리적 이유(애착·상처·두려움) → 그 이야기가 나에게 건네는 위로와 시선 → 오늘의 책(소설) 소개',
    hookExample: '"그 사람은 사랑하면서도, 끝내 곁을 다 내주지 못했습니다"',
    hashtagExample: '#연애소설 #연애심리 #책추천',
  },
};
...
function categoryMood(cat) {
  const c = String(cat || '');
  if (/소설|이야기|로맨스|인물|캐릭터/.test(c)) return 'gentle cinematic storybook mood, warm amber and dusty rose palette, nostalgic soft film-like light, tender narrative atmosphere, like a still from a quiet romance film';
...
const LANE_PAGE1_EMOTION = {
  ...
  story: '이야기 속 그 사람을 애틋하게 바라보는 마음',
};
  ...
  if (step === 1) {
    ...
    if (!bookInfo?.title && state.autoSelect) {
      ...
      if (state.isAutoDaily && bookInfo.title) {
        // 일일 자동: 최근 자동 선정 책만 짧게 기록(신선도 가드) — 며칠 안에 같은 책이 연속
        // 자동 편성되는 것만 막는다. 영구 차단이 아님(게시 안 한 책은 곧 다시 추천 가능).
        try {
          const usedStr = await env.PENDING_POSTS.get('daily_used_books');
          const used = usedStr ? JSON.parse(usedStr) : [];
          await env.PENDING_POSTS.put('daily_used_books', JSON.stringify([...used, bookInfo.title].slice(-14)), { expirationTtl: 6 * 24 * 3600 });
        } catch {}
      }
    }
...
function isTransientPipelineError(msg) {
  const m = String(msg || '');
  if (/ADULT_BLOCKED|SAMESEX_BLOCKED|BUDGET_EXCEEDED|BOOK_NOT_FOUND/.test(m)) return false;
...
async function runDailyAuto(env, slot = 'morning') {
  ...
  // 4레인: 설렘(가벼움)·이별(수익 핵심)·자존감(깊은 위로)·소설(A안, 이야기 속 심리).
  const LANES = ['짝사랑과설렘', '이별과회복', '자존감과사랑', '소설로읽는연애'];
  // 요일별 아침 편성(getDay 0=일). 인접일 같은 레인 금지(주 경계 포함). 이별2·설렘2·자존감2·소설1.
  const DAILY_CATEGORIES = ['짝사랑과설렘', '이별과회복', '소설로읽는연애', '자존감과사랑', '이별과회복', '짝사랑과설렘', '자존감과사랑'];
...
- "transition": 딱 1개. 그런데 어느 날부터 달라졌다는 전환.
...
겉모습은 귀여운 꼬마 강아지인데, 속은 **이 집의 주인**이라고 굳게 믿는다.
인간은 그저 관리인·직원일 뿐이다. 그 시선의 낙차가 웃음을 만든다.

[반드시 지킬 것]
- 한국어 22자 이내, 반말.
- **느낌표를 쓰지 마라.** 담담하게 툭 뱉을수록 웃기다. 소리치면 유치해진다.
- 상황을 **설명하지 마라.** 설명 대신 **판정·선언·통보**로 끝낸다.
- 감탄사·의성어("헉", "우와", "엥")를 쓰지 마라.
- 강아지는 절대 잘못을 인정하지 않는다. 남 탓하거나 사유를 지어낸다.
- 인간에게 존댓말을 쓰지 않는다. 아랫사람 대하듯 한다.
...
- benefit: 광고 문구 금지. **마지못해 인정**하고 생색낸다. ("인정한다", "봐준다", "쓸 만하다")
- cta: 부탁이 아니라 **지시**다. 담담하게 명령한다.
...
function negativeFor(wearing) {
  ...
  const seen = new Set();
  return keep.concat(add).filter(w => {
...
.ref-char[open]>summary::before{content:'− '}
.ref-char textarea{width:100%;box-sizing:border-box;font-size:12px;line-height:1.55;padding:10px;border:1px solid #E2DED6;border-radius:10px;background:#FBFAF7;resize:vertical;font-family:inherit}
...
<main>
  ...
  <section class="box">
    ...
    <div class="f">
      <label for="edVoice">목소리 파일 (선택)</label>
      <input id="edVoice" type="file" accept="audio/*">
      <small>03단계의 내레이션 대본으로 만든 목소리 파일을 넣으면 영상에 함께 얹습니다. 영상 시작과 동시에 재생됩니다.</small>
...
(function(){
  ...
  $('edVoice').addEventListener('change',function(e){
    ...
    if(!f){ $('edVoiceInfo').textContent=''; return; }
    voiceUrl=URL.createObjectURL(f);
    var a=document.createElement('audio'); a.preload='metadata'; a.src=voiceUrl;
    a.addEventListener('loadedmetadata',function(){
      voiceDur=isFinite(a.duration)?a.duration:0;
      var vt=total();
      var msg='목소리 '+fmt(voiceDur)+' · 영상 '+fmt(vt);
      if(vt && voiceDur > vt+0.6) msg+=' — 목소리가 더 깁니다. 뒷부분이 잘립니다.';
      else if(vt && voiceDur < vt-0.6) msg+=' — 목소리가 더 짧습니다. 뒤는 조용해집니다.';
      $('edVoiceInfo').textContent=msg;
    });
  ...
  function playClip(clip,ctx,w,h,ac,dest,onTime,bgGain){
    return new Promise(function(res){
      var v=document.createElement('video');
      ...
      v.addEventListener('canplay',function(){
        if(fin)return;
        try{
          node=ac.createMediaElementSource(v);
          if(bgGain && bgGain.connect){ node.connect(bgGain); } else { node.connect(dest); }
        }catch(e){}
  ...
  $('edGo').addEventListener('click',function(){
    ...
    document.fonts.ready.then(function(){
      ...
      if(voiceUrl){
        ...
        voiceEl.src=voiceUrl; voiceEl.preload='auto';
        voiceEl.style.cssText='position:absolute;left:-9999px;width:1px;height:1px';
        document.body.appendChild(voiceEl);
        voiceElRef=voiceEl;
        try{
          var vSrc=ac.createMediaElementSource(voiceEl);
          var vg=ac.createGain(); vg.gain.value=1;
          vSrc.connect(vg); vg.connect(dest);
        }catch(e){}
      }
