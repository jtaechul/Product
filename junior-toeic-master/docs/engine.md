...
| `AUDIO_RATES` | 0.95 / 1.0 / 1.05 | 듣기 재생 배속 3단계 → 실제 말 속도 약 90 / 95 / 100% (8-1절) |
...
학년 그룹은 **아이 학년(`users.grade`)**으로 결정한다 — 초3~4 = **주니어**, 그 외 = **기본**.
(학년이 없는 옛 학원 계정만 예전처럼 반의 `set_size`로 판정한다. 8-1절 ⚠ 참고.)
...
### 8-1. 듣기 재생 속도 (2026-08-23)

TOEIC Bridge는 실전 자체가 일반 토익보다 느리다. 우리 음원 마스터는 거기서 한 번 더 늦춰
**자연 발화의 95%**로 굽는다(`tools/tts-batch.mjs` RATE). 음원은 **한 벌만** 만들고
재생 배속으로 세 단계를 낸다 — 파일을 세 벌 굽지 않는다(운영비 0원 원칙).

| 단계 | 배속 | 실제 말 속도 | 조건 ('듣고 알기' 점수) |
|---|---|---|---|
| 천천히 | 0.95× | 약 90% | 30점 미만 |
| 기본 | 1.00× | 95% | 30~69점 (측정 전에도 이 값) |
| 실전 | 1.05× | 약 100% | 70점 이상 |

- **경계는 진단 등급 경계와 같다.** 밴드를 이 눈금으로 환산하면 1급 8 / 2급 29 / 3급 50 /
  4급 71 / 5급 92점이라, 30 미만 = 하위 두 등급, 70 이상 = 상위 두 등급이 된다.
  학부모 화면에 보이는 그 숫자와 같은 자를 쓰므로 "72점이라 실전 속도"라고 그대로 설명된다.
- **히스테리시스**: 올릴 땐 70·35, 내릴 땐 65·30. 69↔70을 오가며 매일 속도가 널뛰면
  아이는 "오늘 왜 빨라졌지" 하고 혼란스러워하고, 그 혼란이 오답이 되어 점수를 또 끌어내린다.
  지금 쓰던 값은 `users.audio_rate`에 남긴다.
- **측정 전(3문항 미만)은 무조건 기본.** 모르는 상태에서 빠르게 틀면 실력이 아니라 속도
  때문에 틀리고, 그 오답이 실력 추정을 왜곡한다.
- **초3~4(주니어)는 실전 속도까지 올리지 않는다.** 아홉 살에게 어른 속도는 실력 문제가
  아니라 아직 귀가 못 따라가는 영역이다.
- '천천히' 버튼은 그대로 둔다 — 어떤 단계든 그 문항만 더 느리게(자기 속도의 0.78배) 다시 듣는다.
- 규칙은 `engine.audioRateFor()` **한 곳에만** 있다(순수 함수). 값은 `/api/me`로 한 번
  내려가 앱이 들고 쓴다 — 문항마다 서버에 묻지 않아 망이 느린 곳에서도 속도가 흔들리지 않는다.

**⚠ 학년 그룹 판정 버그 (2026-08-23 함께 수정)** — 이 기능을 붙이며 드러났다.
`groupOf`가 반(`classes.set_size`)으로 주니어/기본을 갈랐는데, 학부모가 만든 아이는 반이 없어
`class_id`가 NULL → 기본값 12 → **모든 아이가 주니어로 판정**되고 있었다. 그래서 잘하는 중3도
듣기 속도가 안 올라가고, 주니어용 짧은 진단(16문항)을 받았다. 원인은 **학년을 아예 수집하지
않은 것** — `createChild`가 `grade`를 받으면서 INSERT에 넣지 않았고 화면도 보내지 않았다.
지금은 가입·아이 추가 화면에서 학년을 받아 `users.grade`에 저장하고 그것으로 그룹을 가른다
(옛 학원 계정은 학년이 없으므로 예전처럼 반으로 판정).
