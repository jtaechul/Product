...
| **축 채우기** | **0~여유만큼** | **0~여유만큼** | 시도 < 3인 **모든 축**을 뒤처진 순서로, 축당 모자란 만큼(≤3문항) 채움 |
...
- **모자란 축 전부**를 뒤처진 순서(시도 적은 순, 같으면 문항 적은 순)로 채운다 (2026-08-23).
  처음엔 가장 뒤처진 축 하나만 채웠는데, 진단이 끝나면 축 두 개(대개 읽고 알기·속뜻 알기)가
  같이 모자라서 하나가 채워지는 동안 다른 하나가 다음 날까지 비었다 —
  "하나씩 실력평가가 빠져 있다"(장하윤 학생의 읽고 알기)가 정확히 이 동작이었다.
...
- 효과(`tools/simulate-axes.mjs`, 학생 30명·21일): 진단 없이 시작해도 **모든 축이
  최악 2일 안에**(20문항 세트는 전부 1일), 진단을 거친 실제 흐름에서는 **첫 세트를 끝내는
  순간 5축 전부** 점수가 나온다(재현 스크립트로 초5·중1 확인).
