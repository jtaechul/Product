...
- 삽입 위치(2026-07-17 개정): ① 영상 설명란 **맨 아래**(줄 띄운 뒤 마지막 줄, 4900자 초과 시에도 항상 덧붙여 잘림 방지)
  ② **고정 댓글** 링크 바로 옆 — 업로드 모듈(M7)에서 코드로 강제, 누락 시 업로드 중단(assert).
  근거: 쇼츠는 설명란이 '더보기'로 접혀 상단/하단 노출 차이가 작고, 시청자가 실제로 보는 1차 표시면은
  링크 옆 고정 댓글 고지다(고정 댓글 assert가 핵심 안전장치). 미관상 설명란 최하단 배치를 사용자가 확정.
...
```json
{
  ...
  "lines": [
    {"text": "빨래를 3분 만에 끝낸다면 믿으시겠습니까", "image_cue": 0, "punch": true},
    {"text": "무게는 고작 1.2킬로그램", "image_cue": 1, "punch": false},
    {"text": "손빨래로 팔 빠지던 시절은 끝났습니다", "image_cue": 0, "punch": false},
    {"text": "이래도 아직 손빨래 하시겠습니까", "image_cue": 0, "punch": false}
  ],
...
- `image_cue`: 해당 라인 시작 시점에 오버레이할 상품 이미지 인덱스 / `punch`: 렌더 쉐이크 트리거(가장 충격적인 훅 라인 1개). **가격은 영상에 절대 미표기(전략 확정) — 링크로만 안내**
...
- 레이어 구성(하→상):
  ...
  4. 쉐이크: `punch=true` 라인(가장 충격적인 훅) 구간 0.3초간 ±8px 랜덤 오프셋
...
작성 규칙
1 훅(첫 라인): 소리내어 3초 내 낭독 가능(15자 내외), 상식파괴형("빨래가 3분 컷?")·결과충격형·의문형 — 이 라인에 "punch": true
2 본문: 셀링포인트 2~3개, 라인당 20자 내외 구어체(위트·되묻기로 리듬), 스펙은 반드시 숫자로 말한다
3 가격 절대 금지: 대사·제목·설명 어디에도 금액(숫자+원, 만원) 미표기 — 가격은 링크로만 안내. "punch": true 라인은 가장 충격적인 훅 1개
4 루프 설계: 마지막 라인의 어미가 첫 라인의 주어로 자연스럽게 이어지도록 작성 (무한 반복 시청 유도)
5 페인포인트: 기존 방식의 불편을 신랄하고 유머러스하게 저격(사람 비하 금지, 상황을 깐다)
6 금지: 절대적 표현(최고, 유일, 100프로), 의학적 효능 단정, 특수기호, 이모지, 영어 남용
7 전체 낭독 분량: 공백 제외 350~450자 (40~55초)
8 image_cue: 각 라인에 어울리는 상품 이미지 인덱스(0부터) 지정

출력 스키마
{"title": "...", "lines": [{"text": "...", "image_cue": 0, "punch": false}], "hashtags": ["...", "...", "..."], "pinned_comment": "...", "description_body": "..."}
```
