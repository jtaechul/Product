...
- **Gemini/Veo 등 유료 AI 영상 생성 API를 절대 호출하지 않는다.** 실제 테스트 결과 품질이
  제작비 대비 값어치가 없다는 사용자 최종 결정. "다시는 제미나이 API 건드리지 마."
- **상품 비주얼은 오직 3가지로만 구성**한다: ① 운영자가 관리자 페이지에서 올린 **제품 영상**
  ② **상품 사진 켄번즈**(줌·팬) 변주 ③ `assets/memes/`의 **밈 이미지**(배경, 글자는 렌더가 얹음).
- 파이프라인(`src/pipeline.py`)에는 Veo 호출이 없고, 워크플로우 env에도 Gemini 키를 주입하지 않는다.
  `src/video/veo.py`는 흔적만 남은 사용 안 하는 모듈 — **재연결·재도입 금지**.
- 예외 없음. 영상을 더 풍성하게 만들 필요가 있으면 유료 생성이 아니라 **운영자 업로드 소스·밈·
  편집 연출**(켄번즈·전환·자막 리듬)로 해결한다.
...
- **비주얼 소싱 부활 (2026-07-13 사용자 확정)**: 과거 '무관한 자동 스톡 폐지'는 **취소**. 문제/훅 씬을
  **문구에 맞는 실사 이미지로 자동 소싱**한다(`src/video/imagesource.py`): 대본 라인 → Claude가 영어
  검색어로 변환 → **대형 무료 라이브러리(Pexels 키 우선 / Openverse=CC·PD 대량)**에서 확보 →
  `scene_images`로 렌더 전달. 실패해도 상품 사진 폴백(빈 화면 없음). 운영자 승인 스톡(`stock_clips`)도
  계속 지원. (저작권은 채널 성장 후 재검토 — 사용자 지시로 지금은 매칭·충실도 우선.)
