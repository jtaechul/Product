...
- 자막 칸은 **대본 생성 시점에 확정**한다: 각 라인의 `subs` 배열(1~3어절, 공백 포함 2~12자,
  ...
  들쭉날쭉 길이의 근본 원인이었다.
- 화면 배치 체계: 일반 자막은 **하단 1곳 고정**(y=1250) / 중앙 밈 카드는 **punch 라인 딱 1회**
  (물음표 자동 트리거 폐지). 그 외 화면 텍스트는 없다(react 추임새 금지 — 위 핵심규칙).
...
- 비주얼: **무관한 자동 스톡 검색 폐지.** 문제 구간(②③)은 어두운 상품 배경 변주, 상품 구간은
  상품 사진 켄번즈 변주(또는 제품 영상 풀프레임). 스톡은 운영자가 후보 그리드에서 승인한 것만
  (`product['stock_clips']`, Phase2 예정).
