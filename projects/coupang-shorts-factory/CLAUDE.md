...
- 웃음도 가드레일 안에서: 사람 비하 금지, §3.3 금지어·의학 단정 금지, 이모지 금지
  (react 추임새는 한글·ㅋㅠ·?!만, 자체 생성 텍스트 밈 = §3.2 화이트리스트).
...
- 화면 배치 체계: 일반 자막은 **하단 1곳 고정**(y=1250) / 중앙 밈 카드는 **punch 라인 딱 1회**
  (물음표 자동 트리거 폐지) / `react` 추임새(ㅋㅋㅋ 등)는 상단 구석 좌우 교대, 대본당 최대 6개.
...
- **Phase 0~3 구현 완료**: M1(아웃라이어 리서치, `shorts-research.yml` 주간) /
  ...
  드립 대본 + `subs` 자막 칸 + `react` 추임새 + punch 1개, 공백 제외 180~280자 잰 템포) /
  M4+M5(TTS 멀티 프로바이더+whisper 폴백 — 실사용: Typecast 원어민, tempo 1.3) /
  M6(렌더: 대본 subs 칸 그대로 하단 고정 팝업(재분할 금지) + punch 밈 카드 1회 + react 스티커 +
  씬 시퀀스(상품 구간=상품 사진 켄번즈 변주·제품 영상 풀프레임, 문제 구간=어두운 상품 배경 변주,
  무관 자동 스톡 폐지) + 자막 스크림·쉐이크. 렌더 후 **통일된 대표 썸네일 `poster.jpg`** —
