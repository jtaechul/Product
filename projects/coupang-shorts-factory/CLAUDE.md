...
> - **차별점 3안 (2026-07-17 사용자 확정)**: "재밌는데 뭘 파는지 모르겠다" 개선. plan이 상품 자료
>   (PDF·캡처 notes)에서 **차별 페인포인트 + 해소 기능·구조 + 작동 방식**을 서로 다른 각도 3안으로
>   추출(`src/product/mechanism.py` → `data/mechanism/{hash}.json`, 워크플로 커밋). 관리자 기획 탭
>   '차별점·작동 방식(3안)' 카드에서 **선택 / 직접 의견(custom, 최우선) / 3안 다시 뽑기** →
>   '이 선택으로 대본 다시 쓰기'(plan 재실행). 확정안은 `product["차별점_작동방식"]`으로 대본에 주입.
