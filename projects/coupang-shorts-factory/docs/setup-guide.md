...
### C-3. 관리자 페이지에서 상품 등록 (링크 + 붙여넣기만)

1. [관리자 페이지](https://shorts-admin.jtaechul.workers.dev) → **상품 탭**
2. **제휴 링크**(C-1) 붙여넣기 + **상품 상세·리뷰 붙여넣기** 칸에 쿠팡 상품 페이지의
   설명과 리뷰 몇 개를 복사해 그대로 붙여넣기 (지저분해도 됨 — 제작 시 자동 정리)
   - 상품명·가격·특징은 붙여넣은 내용에서 **자동 추출**됩니다 (직접 입력은 선택)
