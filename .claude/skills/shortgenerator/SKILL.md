...
- **★코딩 후 실동작·오류 검증(“됐다” 전 필수 · CLAUDE.md 하드룰 #15)**: 코드 고친 뒤 ①import·pytest·문법검사
  (JS `node --check`, YAML 파싱) ②바꾼 경로를 **실제로 한 번 굴려** 확인 ③배포·등록 전제조건 체크
  (**디스패치 워크플로는 main에도 있어야 실행됨**·대시보드는 배포돼야 반영·재생성 캐시 무효화) ④검증한 것과
  못 한 것을 **구분해 정직 보고**. 단위테스트 통과만으로 완료라 말하지 않는다.
